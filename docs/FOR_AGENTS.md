...
- Prefer editing the published app under `site/`; keep `examples/web/` aligned with template CI unless a human expands Golden Path scope.
- Do not reintroduce pruned stacks (android/python/etc.).
- FOSS Cursor hooks/skills/agents/worktrees are active; commercial Cursor docs are not shipped in this child.
