...
- Active stack is **web** only (`site/` PWA + `examples/web/` Golden Path).
- **Design Mode** applies when iterating on `site/` UI in the browser.
- Maintenance board is Sequential-first; open Parallel rows only for non-overlapping feature scopes (`site/**` vs `examples/web/**` vs docs/scripts).
